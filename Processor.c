	...
	switch (operationCode) {
		...
		case OS_INST: // Make a operating system routine in entry point indicated by operand1
			if(Processor_PSW_BitState(EXECUTION_MODE_BIT) == 1){
				...
				ComputerSystem_DebugMessage(69, HARDWARE,InstructionNames[operationCode],
					operand1,operand2,registerPC_CPU,registerAccumulator_CPU,registerPSW_CPU,Processor_ShowPSW());
